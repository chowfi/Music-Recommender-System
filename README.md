...
This collaborative-filter-based music recommender machine is a group project developed by a team of three individuals. The project utilizes implicit feedback from the crowdsourced ListenBrainz dataset. To build and evaluate this recommender machine, we employed distributed computing systems such as NYU High-Performance Computing and Spark.
...
After analyzing the results presented in Table 3, we determined that 𝛽 = 5,000 was the optimal value for the smoothing constant. This selection was based on a high evaluation score, as further increasing the value of 𝛽 only resulted in marginal improvements for both non-cold start and cold-start MAP@100. Although 𝛽 = 1,000,000 achieved the highest validation score, we opted against using it due to the dataset containing 7,909 users, making 𝛽 = 1,000,000 appear excessively large as a smoothing constant. We aimed to avoid introducing significant bias into the model by acknowledging that some songs naturally enjoy higher popularity.
